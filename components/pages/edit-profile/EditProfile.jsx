...
export default function EditProfile({params}) {
  ...
  const updateIndication = useSelector(
    (state) => state.LoginReducer.updateIndication
  );
...
console.error(e)
  ...
  useEffect(() => {
    getProfileDetails();
  }, [updateIndication,CustomUrl]);

  const getProfileDetails = async () => {
    var SendDATA = {
      CustomUrl: UserPayload?.CustomUrl,
      Type: "getProfile",
    };
  ...
  const onChange = (e) => {
    ...
    var formData = { ...userData, ...{ [id]: value } };
  ...
  const Validation = () => {
    let ValidateError = {};
    if (!userData.EmailId) ValidateError.EmailId = "EmailId Required";
    if (userData.EmailId && !Config.EMAIL.test(userData.EmailId))
      ValidateError.EmailId = "Invalid Email Format";
    setErr(ValidateError)
    return ValidateError;
  };

  const FormSubmit = async () => {
    var formData = {...userData,['Type'] : 'profile' };
    ...
    if (!isEmpty(error)) {
      ...
      if (Resp.success === "success") {
        ...
        setDisablebtn(1);
        toast.update(id, {
          render: Resp?.msg ? Resp?.msg : "Validation Failed",
          type: "error",
          isLoading: false,
          autoClose: 1000,
          position: "top-center",
...
>
  ...
  <path d="M19.615 6.25a2.792 2.792 0 00-1.964-1.97C15.905 4 12 4 12 4s-3.905 0-5.651.281A2.792 2.792 0 004.385 6.25C4 8.035 4 12 4 12s0 3.965.385 5.75a2.792 2.792 0 001.964 1.97C8.095 20 12 20 12 20s3.905 0 5.651-.281a2.792 2.792 0 001.964-1.97C20 15.965 20 12 20 12s0-3.965-.385-5.75zM10 15.5v-7l6 3.5-6 3.5z" />
            ...
            <button onClick={FormSubmit} className="rounded-full bg-accent py-3 px-8 text-center font-semibold text-white shadow-accent-volume transition-all hover:bg-accent-dark">
              Update Profile
            </button>
          </div>
